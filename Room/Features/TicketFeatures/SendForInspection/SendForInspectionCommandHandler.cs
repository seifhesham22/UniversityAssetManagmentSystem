...
namespace UAMS.Room.Features.TicketFeatures.SendForInspection
{
    public sealed record SendForInspectionCommand(
        Guid TicketId,
        Guid UserId,
        Guid DepartmentId,
        string? Note) : IRequest;

    internal sealed class SendForInspectionCommandHandler(
        RoomDesignDbContext _db,
        IFacultyFacade _facultyFacade,
        ICampusFacade _campusFacade)
        : IRequestHandler<SendForInspectionCommand>
    {
        public async Task Handle(SendForInspectionCommand request, CancellationToken ct)
        {
            ...
            ticket.SendForInspection(request.UserId, request.DepartmentId, request.Note);
