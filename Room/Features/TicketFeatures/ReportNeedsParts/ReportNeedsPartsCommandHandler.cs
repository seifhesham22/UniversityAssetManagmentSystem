...
namespace UAMS.Room.Features.TicketFeatures.ReportNeedsParts
{
    public sealed record ReportNeedsPartsCommand(
        Guid TicketId,
        Guid MaintainerUserId,
        string Reason) : IRequest;

    internal sealed class ReportNeedsPartsCommandHandler(
        RoomDesignDbContext _db,
        ICampusFacade _campusFacade)
        : IRequestHandler<ReportNeedsPartsCommand>
    {
        public async Task Handle(ReportNeedsPartsCommand request, CancellationToken ct)
        {
            ...
            ticket.ReportNeedsParts(maintainerId, request.Reason);
